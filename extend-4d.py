import argparse
parser = argparse.ArgumentParser(description='Find configurations of points on a 3D grid with distinct distances')
...
N=args.grid_size
pts=args.n_points
a=[[[[(x,y,z,w) for w in range(N)] for z in range(N)] for y in range(N)] for x in range(N)]
a=sum(a,start=[])
a=sum(a,start=[])
a=sum(a,start=[])
...
for u in range(len(a)-1):
    for v in range(u+1,len(a)):
        print("Start ",a[u],a[v]," t=",time.time()-start,"      \r",file=sys.stderr,end='')
        extend([a[u],a[v]],v+1)
